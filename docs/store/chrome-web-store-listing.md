...
## Permissions justification (for the reviewer notes field)
...
```
No analytics, no telemetry, no account, no API key, no backend server. Settings are stored in browser storage. Selected text is sent to the user-configured third-party AI chat (processed under that provider's terms); the extension does not store selected text or replies.
```

See docs/store/privacy-disclosure.md and docs/privacy-policy.md.

## Single purpose statement

```
Rewrite text the user selects in an editable field by sending a rewrite request to the user's own configured AI chat and returning the reply to review and apply.
```
