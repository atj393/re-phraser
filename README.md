...
[![Tests](https://img.shields.io/badge/tests-126-blue)](#testing)
...
https://github.com/user-attachments/assets/2de38951-1d04-43d4-a1a4-1abb6e1c3b37
...
- **Source:** open source under **MIT**, version `1.1.0`.
- **Chrome Web Store:** [live](https://chromewebstore.google.com/detail/re-phraser-ai-text-rewrit/ldocllepggdbadbedboopoeebadnpddi). Edge users install from the same listing.
- **Stack:** TypeScript, React, Vite, Manifest V3.
- **CI:** every push and pull request runs typecheck, 126 unit tests, lint, and a production build.

---

## Quick start

1. **Install** Re-Phraser from the [Chrome Web Store](https://chromewebstore.google.com/detail/re-phraser-ai-text-rewrit/ldocllepggdbadbedboopoeebadnpddi). Edge users can install from the same listing, since there is no separate Edge Add-ons listing yet. To run from source instead, see [For developers](#for-developers).
2. **Open the settings page** by right-clicking the toolbar icon and choosing options, or using the gear button in the floating toolbar.
3. **Open your preferred AI chat** such as ChatGPT, Claude, or Gemini, start a conversation, and send a simple message such as "Hi". Then copy the conversation URL from your browser's address bar.
...
- **About Me:** a short description of who you are and how you like to write, covering tone, language background, and level of formality.
- **Global Prompt:** writing rules you want applied to every rewrite, for example "keep it concise" or "use plain English".
- **Avoid These Things:** things you never want, for example "avoid clichés" or "do not add information I did not write".
...
The GitHub repository is Re-Phraser's home and support channel. Please use **[GitHub Issues](https://github.com/atj393/re-phraser/issues)** for questions and bug reports. See [SUPPORT.md](SUPPORT.md) for how to write a useful report.
...
**Permissions, and why each is needed:** `storage` for settings, `tabs` to find and
reuse the configured chat tab, `clipboardWrite` as a fallback when the reply cannot
be inserted, and an `<all_urls>` content script so the toolbar can appear in any
...
**No network calls at all.** There is no `fetch`, no `XMLHttpRequest`, and no
WebSocket anywhere in `src/`. Everything the extension does is DOM work in tabs
...
| `popupHostname` | 18 | Per-site enable and disable matching rules |
...
| `validation` | 7 | AI chat URL validation, http and https only |
...
directly. Not covered: live rewrites against a real ChatGPT, Claude, or Gemini tab.
